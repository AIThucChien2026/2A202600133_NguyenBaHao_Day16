from __future__ import annotations

from dataclasses import dataclass
from typing import Literal

from .llm_runtime import actor_answer, evaluator, reflector
from .schemas import AttemptTrace, QAExample, ReflectionEntry, RunRecord


def _classify_failure(reason: str) -> str:
    """Classify failure mode from evaluator reason text."""
    r = reason.lower()
    if "hop" in r or "incomplete" in r or "partial" in r:
        return "incomplete_multi_hop"
    if "drift" in r or "wrong entity" in r:
        return "entity_drift"
    if "loop" in r or "repeat" in r:
        return "looping"
    if "overfit" in r or "same mistake" in r:
        return "reflection_overfit"
    return "wrong_final_answer"


@dataclass
class BaseAgent:
    agent_type: Literal["react", "reflexion"]
    max_attempts: int = 1

    def run(self, example: QAExample) -> RunRecord:
        reflection_memory: list[str] = []
        reflections: list[ReflectionEntry] = []
        traces: list[AttemptTrace] = []
        final_answer = ""
        final_score = 0
        last_reason = ""

        for attempt_id in range(1, self.max_attempts + 1):
            # --- Actor: trả lời câu hỏi (Groq LLM) ---
            answer, actor_tokens, actor_latency = actor_answer(
                example, attempt_id, self.agent_type, reflection_memory
            )

            # --- Evaluator: chấm điểm (qwen/qwen3-32b) ---
            judge, eval_tokens, eval_latency = evaluator(example, answer)

            # Token + latency thực tế từ API
            token_estimate = actor_tokens + eval_tokens
            latency_ms = actor_latency + eval_latency

            trace = AttemptTrace(
                attempt_id=attempt_id,
                answer=answer,
                score=judge.score,
                reason=judge.reason,
                token_estimate=token_estimate,
                latency_ms=latency_ms,
            )

            final_answer = answer
            final_score = judge.score
            last_reason = judge.reason

            if judge.score == 1:
                traces.append(trace)
                break

            # --- Reflector: phân tích lỗi + chiến thuật (qwen/qwen3-32b) ---
            if self.agent_type == "reflexion" and attempt_id < self.max_attempts:
                ref, ref_tokens, ref_latency = reflector(example, attempt_id, judge)
                trace.reflection = ref
                trace.token_estimate += ref_tokens
                trace.latency_ms += ref_latency
                reflections.append(ref)
                reflection_memory.append(ref.next_strategy)

            traces.append(trace)

        total_tokens = sum(t.token_estimate for t in traces)
        total_latency = sum(t.latency_ms for t in traces)
        failure_mode = (
            "none" if final_score == 1 else _classify_failure(last_reason)
        )

        return RunRecord(
            qid=example.qid,
            question=example.question,
            gold_answer=example.gold_answer,
            agent_type=self.agent_type,
            predicted_answer=final_answer,
            is_correct=bool(final_score),
            attempts=len(traces),
            token_estimate=total_tokens,
            latency_ms=total_latency,
            failure_mode=failure_mode,
            reflections=reflections,
            traces=traces,
        )


class ReActAgent(BaseAgent):
    def __init__(self) -> None:
        super().__init__(agent_type="react", max_attempts=1)


class ReflexionAgent(BaseAgent):
    def __init__(self, max_attempts: int = 3) -> None:
        super().__init__(agent_type="reflexion", max_attempts=max_attempts)

    def run(self, example: QAExample) -> RunRecord:
        # Tự động điều chỉnh số lần thử tùy theo độ khó (Adaptive Max Attempts)
        if example.difficulty == "easy":
            self.max_attempts = 1
        elif example.difficulty == "medium":
            self.max_attempts = 3
        else:  # hard
            self.max_attempts = 5
            
        return super().run(example)

